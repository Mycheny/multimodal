...
def read_submission(submit_path, reference, k=5):
    ...
    if len(nopred_qids) != 0:
        [reference.pop(k) for k in nopred_qids]
        return submission_dict, reference
...
if __name__ == "__main__":
    ...
    standard_path = "E:/DATA/multimodal/multimodal_valid/valid_answer.json"
    # the path of prediction file (csv or zip)
    # submit_path = sys.argv[2]
    submit_path = "E:/DATA/multimodal/multimodal_submit_example_testA/submission.csv"
    ...
    try:
        ...
        # read predictions
        k = 5
        result = read_submission(submit_path, reference, k)
        if isinstance(result, tuple):
            predictions, reference = result
        else:
            predictions = result
        print()
        reference = {
